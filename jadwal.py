...
success = False
fitness = pymp.shared.array((jml_kromosom,), dtype=object)
total_fitness = 0
...
best_fitness = 0
probability = []
crommosom = pymp.shared.array((jml_kromosom,), dtype=object)
generataion = 0
max_generataion = 10
timeClash = {}
objectFitnes = {}
...
def isCheckTimeClash(gen1, gen2):
    ...
    if clash1 not in timeClash:
        ...
        if gen1['waktu'] == gen2['waktu']:
            ...
            sks1 = int(kuliah[gen1['kuliah']][5])
            sks2 = int(kuliah[gen2['kuliah']][5])
...
def checkPreferensiDosen(dosen, waktu):
    if dosen in preferensi_dosen:
        return waktu in preferensi_dosen[dosen]
    return False
...
def checkClashDosen(_cromosom):
    ...
    for i in range(len(_cromosom)-1):
        for j in range(i+1, len(_cromosom)):
            dosen1 = kuliah[_cromosom[i]['kuliah']][2]
            dosen2 = kuliah[_cromosom[j]['kuliah']][2]
            if dosen1 == dosen2:
                if isCheckTimeClash(_cromosom[i], _cromosom[j]) or not checkPreferensiDosen(dosen1, _cromosom[i]['waktu']):
                    result[i] = i
    return result
...
def mutation():
    ...
    with pymp.Parallel(32) as paralel:
        for _ in paralel.range(1, totalMutation + 1):
